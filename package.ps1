...
if (-not $artifactsDirectory.StartsWith($expectedPrefix, [StringComparison]::OrdinalIgnoreCase)) {
    throw 'Artifacts directory escaped the plugin repository.'
}
...
$manifest = Get-Content -LiteralPath (Join-Path $projectRoot 'plugin.json') -Raw -Encoding UTF8 |
    ConvertFrom-Json
if ($manifest.id -ne 'io.github.touristh.example-clock' -or $manifest.version -ne '1.0.0') {
    throw 'plugin.json identity does not match this release.'
}
...
$unexpectedSdk = Get-ChildItem -LiteralPath $packageDirectory -Recurse -File |
    Where-Object Name -IEq 'NyaLauncher.Plugin.Abstractions.dll'
if ($unexpectedSdk) {
    throw 'The host-provided SDK assembly must not be packaged.'
}

$archive = Join-Path $artifactsDirectory 'io.github.touristh.example-clock-1.0.0.zip'
